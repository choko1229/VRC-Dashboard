...
- **フレンド状況**: VRChatの認証情報を保存し、Pipeline（WebSocket）でフレンドのオンライン/
  ...
  - フレンド一覧ページ（`/friends`）は絞り込みタブなしで、「お気に入り／オンライン／
    ...
    公開範囲（人数）」の見出し、人数の多いグループ順）、インスタンスが不明（プライベート等）
    なフレンドはグループの後に見出し無しで続けて表示する（別区分には分割しない）。
    ワールドが判明しているフレンドのカードは、そのワールドのサムネイル画像を背景に表示する
    （`friend.current_world_thumbnail_url`。Pipelineのfriend-online/friend-locationイベントの
    worldオブジェクトからのみ取得できるため、REST同期直後はまだ設定されない場合がある）。
    カードは内容の有無によらず同じ大きさに揃えている。カードをクリックするとページ遷移せず
    モーダルでフレンド詳細を表示する。
